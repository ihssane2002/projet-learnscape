...
let timer1, timer2, timer3; // Déclaration des variables timer pour chaque quiz
...
async function verifyCode(quizCode, codeId) {
    ...
    try {
        ...
        const snapshot = await get(child(dbRef, `Cours Magistral 2/Quizzes/${codeId}`));
...
document.addEventListener('DOMContentLoaded', () => {
    // Écouter l'événement "click" sur le bouton "Participer" du Quiz 5
    document.getElementById('submit-code5').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input5').value;
        console.log('Bouton "Participer" du Quiz 5 cliqué');
        console.log('Code ID entré pour le Quiz 5:', codeId);
        const quizData = await verifyCode('Quiz 5', codeId);
        if (quizData) {
            alert('Code ID valide pour le Quiz 1');
            displayQuiz('quiz-container5', quizData);
        } else {
            alert('Code ID invalide pour le Quiz 1');
        }
    });

    // Écouter l'événement "click" sur le bouton "Participer" du Quiz 6
    document.getElementById('submit-code6').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input6').value;
        console.log('Bouton "Participer" du Quiz 6 cliqué');
        console.log('Code ID entré pour le Quiz 6:', codeId);
        const quizData = await verifyCode('Quiz 6', codeId);
        if (quizData) {
            alert('Code ID valide pour le Quiz 2');
            displayQuiz('quiz-container6', quizData);
        } else {
            alert('Code ID invalide pour le Quiz 2');
        }
    });

    // Écouter l'événement "click" sur le bouton "Participer" du Quiz 7
    document.getElementById('submit-code7').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input7').value;
        console.log('Bouton "Participer" du Quiz 7 cliqué');
        console.log('Code ID entré pour le Quiz 7:', codeId);
        const quizData = await verifyCode('Quiz 7', codeId);
        if (quizData) {
            alert('Code ID valide pour le Quiz 3');
            displayQuiz('quiz-container7', quizData); // Correction du conteneur ici
        } else {
...
function displayQuiz(containerId, quizData) {
    ...
    if (containerId === 'quiz-container5') {
        timer1 = startQuizTimer(timerElement, 'Quiz 5');
    } else if (containerId === 'quiz-container6') {
        timer2 = startQuizTimer(timerElement, 'Quiz 6');
    } else if (containerId === 'quiz-container7') {
        timer3 = startQuizTimer(timerElement, 'Quiz 7');
    }
...
async function submitQuiz(containerId) {
    let timer;
    if (containerId === 'quiz-container5') {
        clearInterval(timer1); // Arrêter le minuteur du Quiz 5
        timer = timer1;
    } else if (containerId === 'quiz-container6') {
        clearInterval(timer2); // Arrêter le minuteur du Quiz 6
        timer = timer2;
    } else if (containerId === 'quiz-container7') {
        clearInterval(timer3); // Arrêter le minuteur du Quiz 7
        timer = timer3;
    }
    ...
    const quizCode = containerId === 'quiz-container5' ? document.getElementById('code-input5').value :
                      containerId === 'quiz-container6' ? document.getElementById('code-input6').value :
                      document.getElementById('code-input7').value;
