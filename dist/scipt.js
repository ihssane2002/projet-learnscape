...
// Définir les détails de la question
const quizCode = 'ALKGo1';
// Données des questions à ajouter
const questionData = {
 "question1": {
    "question": "Quelle est l'unité de la contrainte dans le système international (SI) ?",
    "answers": {
        "A": "Pascal (Pa)",
        "B": "Newton par mètre carré (N/m²)",
        "C": "Kilogramme par mètre cube (kg/m³)",
        "D": "Joule par mètre carré (J/m²)"
    },
    "correctAnswer": "Pascal (Pa)"
},
"question2": {
    "question": "Quelle est l'unité de la viscosité dynamique dans le système international (SI) ?",
    "answers": {
        "A": "Pascal-seconde (Pa·s)",
        "B": "Newton par mètre carré (N/m²)",
        "C": "Pascal par mètre carré (Pa/m²)",
        "D": "Joule par mètre carré (J/m²)"
    },
    "correctAnswer": "Pascal-seconde (Pa·s)"
}

};

// Référence à l'emplacement spécifique dans Firebase Realtime Database
const quizRef = ref(database, `Cours Magistral 1/Quizzes/${quizCode}`);

// Ajouter les questions à Firebase Realtime Database
set(quizRef, questionData)
    .then(() => {
        console.log('Questions ajoutées avec succès à Firebase Realtime Database.');
    })
    .catch((error) => {
...
async function verifyCode(quizCode, codeId) {
    ...
    try {
        ...
        const snapshot = await get(child(dbRef, `Cours Magistral 1/Quizzes/${codeId}`));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('submit-code1').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input1').value;
    ...
    document.getElementById('submit-code2').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input2').value;
...
document.addEventListener('DOMContentLoaded', () => {
    // Écouter l'événement "click" sur le bouton "Participer" du Quiz 1
    document.getElementById('submit-code3').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input3').value;
    ...
    document.getElementById('submit-code4').addEventListener('click', async (event) => {
        const codeId = document.getElementById('code-input4').value;
...
function displayQuiz(containerId, quizData) {
    ...
    if (containerId === 'quiz-container1') {
        timer1 = startQuizTimer(timerElement, 'Quiz 1');
    } else if (containerId === 'quiz-container2') {
        timer2 = startQuizTimer(timerElement, 'Quiz 2');
    } else if (containerId === 'quiz-container3') {
        timer3 = startQuizTimer(timerElement, 'Quiz 3');
    }else if (containerId === 'quiz-container4') {
...
async function submitQuiz(containerId) {
    ...
    const quizCode = containerId === 'quiz-container1' ? document.getElementById('code-input1').value :
                      containerId === 'quiz-container2' ? document.getElementById('code-input2').value :
                      containerId === 'quiz-container3' ? document.getElementById('code-input3').value :
                      document.getElementById('code-input4').value;
